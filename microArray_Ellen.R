...
## filter the p value < 0.1 due to the limited data.
## order by logFC > 1 to see the positive up-regulated genes.
ressig <- topTable(fit, coef=2, adjust="BH", p.value = 0.1, number = 1000) 
upreg <- ressig %>% filter( logFC >= 1)  %>% arrange(logFC)
upreg
